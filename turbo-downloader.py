...
def https_get(url, headers={}, nobody=False):
    c = pycurl.Curl()
...
def get_chunk(url, OUTPUT_FILE, offset, length, index):
    ...
    r = https_get(url, headers={'Range': 'bytes=%d-%d' % (offset, offset + length - 1)})
    if r['status'] != 206:
    ...
    write_index += 1
    running -= 1
r = https_get(args.url, nobody=True)
if r['status'] != 200:
    raise Exception('Unexpected status %d' % r['status'])
...
if not OUTPUT_FILENAME:
    OUTPUT_FILENAME = os.path.basename(urllib.parse.urlparse(args.url).path)
...
while offset <= TOTAL_SIZE:
    running += 1
    threading.Thread(target=get_chunk, args=(args.url, OUTPUT_FILE, int(offset), CHUNK_SIZE, int(index)), daemon=True).start()
    index += 1
    offset += CHUNK_SIZE
